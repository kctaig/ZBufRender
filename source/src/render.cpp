        ...
        for (int i = 0; i < triangles.size(); ++i) {
            ...
            for (int x = bbox.getMinX(); x < bbox.getMaxX(); x++) {
                for (int y = bbox.getMinY(); y < bbox.getMaxY(); y++) {
                    auto depth = calculateDepth({x, y}, localFragMesh);
...
void Render::naiveHierarchyRender(const Shader& shader,
    ...
    */
    ...
    for (int i = 0; i < triangles.size(); ++i) {
        ...
        naiveHierarchyBufferPtr->getRoot()->checkFragMesh(fragMesh);
    }
}

float Render::calculateDepth(glm::ivec2 pixel,
