...
1. **初始化四叉树和八叉树**：
    - **四叉树的初始化**：
      ...
      + 递归地将每个子节点继续划分为四个更小的部分,直到达到预设的最大深度或者节点的尺寸小于一个特定的阈值。
    - **kd 树的初始化**：
      + 对于场景中的每个 `Mesh`,执行以下步骤：
      + 计算该 Mesh 在三维空间中的包围盒（Bounding Box）。
      + 从 kd 树的根节点开始,递归地将 `Mesh` 分配到其所属的最深的 kd 树节点中。
      + 如果一个 kd 树节点包含多个 `Mesh`,并且节点的大小允许进一步分割,则将该节点分割成更小的 kd 树节点,直到每个节点只包含一个 `Mesh` 或者达到 kd 树的最大深度。
    - **绘制场景**：
      + 建立一个栈，将四叉树根节点加入到栈中，然后开始循环，每轮循环取出一个节点，检查 kd 树节点
      + 
