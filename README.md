...
1. **初始化四叉树和八叉树**：
    - **四叉树的初始化**：同以上构建四叉树方法相同。
    - **kd 树的初始化**：
      +  选择一个维度（轴）进行空间划分。
      +  根据所选轴上的坐标值，将网格（mesh）中的点分配到相应的KD树子节点中；若点不明确属于任一子节点，则将其归类到当前节点。
      +  递归执行上述划分过程，直到达到预定的最大深度或节点的体积小于给定的阈值。
    - **绘制场景**：从四叉树的根节点开始，同时查看 kd 树根节点，递归的进行以下操作。
     + 
